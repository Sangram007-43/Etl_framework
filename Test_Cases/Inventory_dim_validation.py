...
logger = Logs.Log_Gen(f'C:\\Users\\Sangram\\PycharmProjects\\Configdrivenapproach\\Logs\\inventory_dim_{dt}.log')

with open(r'C:\Users\Sangram\PycharmProjects\Configdrivenapproach\Config\SQL_Queries_config\Inventory_dim.json','r') as Inventory_data_SQL_fle:
    Inventory_SQL_Queries = json.load(Inventory_data_SQL_fle)

# Retrieve dynamic table names
source_table_name = Inventory_SQL_Queries["table_names"]["source"]
target_table_name = Inventory_SQL_Queries["table_names"]["target"]

def Source_Target_Count_check(source_db_conn,target_db_conn,Inventory_SQL_Queries,logger):
    source_result = pd.read_sql(Inventory_SQL_Queries['count_comparison']['source_query'],source_db_conn)
    target_result = pd.read_sql(Inventory_SQL_Queries['count_comparison']['target_query'],target_db_conn)
...
#Defining a format of output result to write into excel file
    ...
    logger.info(f"Source Inventory table Count:{source_count}")
    logger.info(f"Target Inventory_dim table Count: {target_count}")
...
def Null_Checks(target_db_conn,Inventory_SQL_Queries,logger):
    target_result = pd.read_sql(Inventory_SQL_Queries['null_check']['target_query'],target_db_conn)
    ...
    if null_count == 0:
        ...
        null_records_query = Inventory_SQL_Queries['null_check']['null_records_query']
...
def Duplicate_Records_chk(target_db_conn,Inventory_SQL_Queries,logger):
    tgt_result = pd.read_sql(Inventory_SQL_Queries["duplicate_check"]["target_query"],target_db_conn)
    ...
    if duplicate_count == 0:
        ...
        duplicate_records_query = Inventory_SQL_Queries["duplicate_check"]["duplicate_records_query"]
...
def Column_mapping_Validation(source_db_conn, target_db_conn, Inventory_SQL_Queries, logger):
    source_result = pd.read_sql(Inventory_SQL_Queries["column_mapping"]["source_query"], source_db_conn)
    target_result = pd.read_sql(Inventory_SQL_Queries["column_mapping"]["target_query"], target_db_conn)
...
counts = Source_Target_Count_check(source_db_conn,target_db_conn,Inventory_SQL_Queries,logger)
null_chk = Null_Checks(target_db_conn,Inventory_SQL_Queries,logger)
duplicates = Duplicate_Records_chk(target_db_conn,Inventory_SQL_Queries,logger)
columns_mapp = Column_mapping_Validation(source_db_conn,target_db_conn,Inventory_SQL_Queries,logger)
...
dt = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
output_file_path = f"C:\\Users\\Sangram\\PycharmProjects\\Configdrivenapproach\\Output_Result\\Inventory_dim_Result_{dt}.xlsx"
