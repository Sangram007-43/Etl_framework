...
logger = Logs.Log_Gen(f'C:\\Users\\Sangram\\PycharmProjects\\Configdrivenapproach\\Logs\\Cost_dim_{dt}.log')

with open(r'C:\Users\Sangram\PycharmProjects\Configdrivenapproach\Config\SQL_Queries_config\Cost_dim.json','r') as Cost_data_SQL_fle:
    Cost_SQL_Queries = json.load(Cost_data_SQL_fle)

# Retrieve dynamic table names
source_table_name = Cost_SQL_Queries["table_names"]["source"]
target_table_name = Cost_SQL_Queries["table_names"]["target"]
...
def Null_Checks(target_db_conn,Cost_SQL_Queries,logger):
    target_result = pd.read_sql(Cost_SQL_Queries['null_check']['target_query'],target_db_conn)
    ...
    if null_count == 0:
        ...
        null_records_query = Cost_SQL_Queries['null_check']['null_records_query']
...
def Duplicate_Records_chk(target_db_conn,Cost_SQL_Queries,logger):
    tgt_result = pd.read_sql(Cost_SQL_Queries["duplicate_check"]["target_query"],target_db_conn)
    ...
    if duplicate_count == 0:
        ...
        duplicate_records_query = Cost_SQL_Queries["duplicate_check"]["duplicate_records_query"]
...
def Column_mapping_Validation(source_db_conn, target_db_conn, Cost_SQL_Queries, logger):
...
dt = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
output_file_path = f"C:\\Users\\Sangram\\PycharmProjects\\Configdrivenapproach\\Output_Result\\Cost_dim_Result_{dt}.xlsx"
